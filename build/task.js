...
const buildImage = (srcPath, distPath) => () => src(srcPath).pipe(imagemin()).pipe(dest(distPath))

const copyStatic = (srcPath, distPath, env = 'build') => {
	if (env === 'build') {
		return parallel(copy(srcPath, distPath, 'wxs'))
	} else {
		return parallel(
...
const clean = (cleanPath) => () => del([ cleanPath ], { force: true })
